...
public class UserIdHandshakeInterceptor implements HandshakeInterceptor {
    ...
    public boolean beforeHandshake(
        ...
        if (StringUtils.hasText(userId)) {
            attributes.put(IdentityWsConstants.USER_ID_SESSION_ATTR, userId);
        }
        return true;
    }
    ...
    private static String extractSubFromBearerToken(String authorization) {
        ...
        return extractSubFromJwt(authorization.substring(BEARER_PREFIX.length()).trim());
    }

    static String extractSubFromJwt(String token) {
