...
export default function ChatWindow() {
  ...
  const loadCurrentTag = useCallback(async () => {
    if (!peerUserId || !currentUserId) {
      setCurrentTag(null)
      return
    }
    try {
      const tagRes = await tagService.getTagsByUserId(peerUserId)
      const peerTag = tagRes.data
      if (peerTag && peerTag.taggerId === currentUserId) {
